  ...
  for (const pdf of pdfs) {
    ...
    if (pdf.selectedPages && pdf.selectedPages.size > 0) {
      ...
      try {
        ...
        console.error(
          `Error loading page content from PDF: ${pdf.fileName}`,
          error
        );
      } finally {
        loadingTask.destroy();
...
const generateConversationLists = (currentMessage, messagesTree) => {
  ...
  otherMessagesList = otherMessagesList.filter(
    (msg) => !conversationList.includes(msg)
  );
...
const getConversationHistory = (currentMessage, messagesTree) => {
  const { conversationList, otherMessagesList } = generateConversationLists(
    currentMessage,
    messagesTree
  );
...
const callApiNoStream = async (fullPrompt, useApi) => {
  try {
    const resp = await fetch(
      "https://api.promptperfect.jina.ai/tcok9OGA8BjFBzPp2KqR",
      {
        headers: {
...
const callApi = async (fullPrompt, useApi, updateBotMessage) => {
  ...
  try {
    ...
    const resp = await fetch(
      "https://api.promptperfect.jina.ai/RlYg9Ir63bt7Sr1w1b34",
      {
        headers: {
          "x-api-key": `token ${YOUR_GENERATED_SECRET}`,
          "content-type": "application/json",
...
const getBotReply = async (
  ...
  // console.log(
    // "SDHFSDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD"
  // );
  let fullPrompt = "";
...
const getContextTokens = async (currentMessage, messagesTree, pdfs) => {
  ...
  const tokenCount = Math.ceil(
    words.length + fullPrompt.length / averageCharactersPerWord
  );
