...
const SideNavBar = ({
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3001/api/contexts")
  ...
  const handleSendClick = async () => {
    try {
      const response = await BotFunctions.callApiContext(
        chatInput,
        currentMessage,
        messageTree,
        pdfs,
  ...
  return (
    <div className={`side-nav ${isCollapsed ? "collapsed" : ""}`}>
      ...
      <div className="nav-item">
        ...
        <textarea
          className="nav-textarea custom-scrollbar"
          placeholder="Enter chat context"
          readOnly
          value={contextualChatArea}
          onChange={(e) => setContextualChatArea(e.target.value)}
        ></textarea>
      ...
      <div className="nav-item">
        ...
        <button
          style={buttonStyles}
          className="nav-button"
          onClick={() => handleSaveContext(contextInput)}
        >
          Save Context
        </button>
          ...
          {contextList.map((contextName, index) => (
            <li
              ...
              className={`nav-list-item ${
                contextName === currentContext ? "current-context" : ""
              }`}
              onClick={() => {
