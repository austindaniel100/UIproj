...
const ChatComponent = ({
  ...
  const isOtherMessagesListArray =
    Array.isArray(otherMessagesList) && otherMessagesList.length > 0;
  ...
  return (
    ...
    >
      {isOtherMessagesListArray && (
        <div
          className="other-messages-column"
        ...
        >
          ...
          {otherMessagesList.map((messageNode) => (
            <MessageComponent
              ...
              isCurrentBotMessage={
                currentBotMessage && messageNode.id === currentBotMessage.id
              }
            />
          ))}
        </div>
      ...
      <div
        className="conversation-list-column"
        style={{ flexGrow: 1, padding: "0 10px" }}
      >
        <h2 style={titleStyle}>Conversation</h2>
        <div style={{ width: "100%" }}>
          {isConversationListArray &&
            conversationList.map((messageNode) => (
              <div style={{ width: "100%", flexWrap: "wrap" }}>
                <MessageComponent
                  key={messageNode.id}
                  messageNode={messageNode}
                  isCurrentMessage={
                    currentMessage && messageNode.id === currentMessage.id
                  }
