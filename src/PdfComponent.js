...
const PdfViewerComponent = forwardRef(
  ({ pdfs, setPdfs, setTokenCount, input, currentMessage, messages }, ref) => {
    ...
    const onDocumentLoadSuccess = (pdfIndex, { numPages }) => {
      setPdfs((prevPdfs) =>
        prevPdfs.map((pdf, index) =>
          index === pdfIndex ? { ...pdf, numPages } : pdf
        )
      );
    };
    ...
    const calculateOverlayHeight = (pdf) => {
      ...
      const percentage =
        totalPages > 0 ? (selectedPages / totalPages) * 100 : 0;
      return `${percentage}%`;
    };
          ...
          <div>
            {pdfs.map((pdf, index) => (
              <div
                ...
                style={{ ...pdfThumbnailStyle, position: "relative" }}
              >
                <div
                  style={{
                    position: "absolute",
                ...
                ></div>
                <div style={fileNameStyle}>{pdf.fileName}</div>
              </div>
            ))}
          </div>
          {pdfs.map((pdf, index) => (
            ...
            >
              <button
                onClick={() => setOpenPopupIndex(-1)}
                style={customButtonStyle}
              >
                Close
              </button>
              <button
                style={{ ...customButtonStyle, ...darkModeStyles }}
              ...
              <div style={{ display: "flex", justifyContent: "center" }}>
                ...
                <div>
                  {hoveredPage.pdfIndex === index && (
                    <Document
                      style={documentStyle}
                      file={pdfs[hoveredPage.pdfIndex].file}
                      onLoadSuccess={(event) =>
                        onDocumentLoadSuccess(index, event)
                      }
                    >
                      <div
                        ...
                        style={{
                          ...largeViewContainerStyle,
                          ...pdfContainerStyle,
                        }}
                      >
                        {" "}
                        {/* Apply the filter style here */}
                        <Page
