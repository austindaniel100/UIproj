    ...
    g.selectAll(".click-capture")
      ...
      .on("click", (event, node) => {
        if (event.ctrlKey) {
          ...
          BotFunctions.getContextTokens(currentMessage, messages, pdfs).then(
            (tokens) => {
...
const PromptPopup = React.forwardRef(
  (
    {
      onClose,
      onSend,
    ...
    const updateDataString = (name, value) => {
      setDataStrings((prevStrings) =>
        prevStrings.map((ds) =>
          ds.name === name ? { ...ds, content: value } : ds
        )
      );
    };
    ...
    useEffect(() => {
      let sortedPrompts = prompts.filter(
        (p) =>
    ...
    const handleOverwriteConfirmation = (overwrite) => {
      if (overwrite) {
        const updatedPrompts = prompts.map((p) =>
          p.name === overwritePromptName
            ? { ...p, content: inputValue, parameters }
            : p
        );
    ...
    const dataStringRefs = useMemo(
      () => parseDataStringReferences(inputValue),
      [inputValue, dataStringDict]
    );

    const handleDataStringChange = (dataStringName, value) => {
      setDataStringDict((prevDict) => ({
        ...prevDict,
        [dataStringName]: value,
      }));
    };
      ...
      while ((match = regex.exec(inputValue)) !== null) {
        if (dataStrings.some((d) => d.name === match[1])) {
          ...
          dataStringInputs.push(
            <div
              key={dataStringName}
              onClick={() => dataStringName && openPopup(dataStringName)}
            >
              ...
              <input
                ...
                onChange={(e) =>
                  handleDataStringChange(match[1], e.target.value)
                }
    ...
    const DataStringPopup = ({ isOpen, onClose, onSave, initialValue }) => {
      ...
      return (
        <div
          style={{ ...popupStyle, display: "flex", justifyContent: "center" }}
          onClick={onClose}
        >
          <div
            style={{
      ...
      <div ref={ref} style={popupContainerStyle}>
        ...
        <div style={{ flex: 2, padding: "20px" }}>
          ...
          <div style={buttonsContainerStyle}>
            <Button
              onClick={handleSavePromptWithParameters}
              style={buttonStyles}
            >
              Save Prompt
            </Button>
        ...
        {showConfirmOverwrite && (
          <div style={confirmOverwriteStyle}>
            ...
            <div style={{ display: "flex", justifyContent: "center" }}>
              <Button
                onClick={() => handleOverwriteConfirmation(true)}
                autoFocus
                style={buttonStyles}
              >
                Yes
              ...
              <Button
                onClick={() => handleOverwriteConfirmation(false)}
                style={buttonStyles}
              >
                No
              </Button>
        ...
        <DataStringPopup
          ...
          initialValue={
            dataStrings.find((ds) => ds.name === currentDataString)?.content ||
            ""
          }
        />
      </div>
...
const DataPopup = React.forwardRef(
  (
    {
      onClose,
      onDataSave,
    ...
    const handleDataSave = () => {
      ...
      const existingDataIndex = dataStrings.findIndex(
        (d) => d.name === dataName
      );
    ...
    const listItemContainerStyle = (data, index) => ({
      ...
      justifyContent: "space-between",
      alignItems: "center",
      backgroundColor: systemPrompt?.includes(data.content)
        ? "#4d4000"
        : hoverIndex === index
        ? "#484848"
        : "#333",
      ...
      <div ref={ref} style={popupContainerStyle}>
        ...
        <div style={{ ...detailSectionStyle, background: "transparent" }}>
          ...
          <div style={{ display: "flex", gap: "10px" }}>
            ...
            <Button onClick={handleDataSave} style={buttonStyles}>
              Save Data
            </Button>
            <Button onClick={handleSetSystemPrompt} style={goldButtonStyle}>
              Set System Prompt
            </Button>
          </div>
        </div>
...
`;
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        dataPopupRef.current &&
        !dataPopupRef.current.contains(event.target)
      ) {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        promptPopupRef.current &&
        !promptPopupRef.current.contains(event.target)
      ) {
  ...
  const saveChatContext = async (contextName) => {
    ...
    const serializeMessageNodes = (node, parentId = null) => {
      const serializedNode = {
        ...
        children: node.children.map((child) =>
          serializeMessageNodes(child, node.id)
        ),
      };
    ...
    try {
      ...
      const pdfFileReferences = await Promise.all(
        pdfs.map(async (pdf) => {
  ...
  const loadChatContext = async (contextName) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/api/loadContext/${contextName}`
      );
      if (!response.ok) {
        throw new Error(
          `Error loading chat context '${contextName}': ${response.statusText}`
        );
      }
      ...
      const pdfs = await Promise.all(
        pdfFileReferences.map(async (fileRef) => {
          ...
          return new Promise((resolve, reject) => {
            ...
            reader.onload = (e) =>
              resolve({
                ...
                numPages: fileRef.numPages,
                selectedPages: new Set(fileRef.selectedPages || []),
      ...
      const rebuildMessageNodes = (node, allNodes, parent = null) => {
        ...
        newNode.children = (node.children || []).map((childId) =>
          rebuildMessageNodes(allNodes[childId], allNodes, newNode)
        );
        return newNode;
      };
      ...
      let rebuiltMessageTree = rebuildMessageNodes(
        allNodes[messageTree.id],
        allNodes
      );
      ...
      if (pdfName.toLowerCase().startsWith(inputName.toLowerCase()[0])) {
        ...
        if (inputWords.length > 1) {
          ...
          for (let i = 1; i < inputWords.length; i++) {
            if (
              pdfWords.length > i &&
              pdfWords[i].startsWith(inputWords[i][0])
            ) {
  ...
  const handlePdfCommand = async (input = "") => {
    ...
    const pdfCommands = [
      { command: "view", description: "view(name) the chosen pdf name" },
      {
        command: "select",
        description: "select(pages) selects the number of pages",
      },
      { command: "deselect", description: "deselect page numbers for a pdf" },
      { command: "clear", description: "clear all pdfs" },
      { command: "remove", description: "remove named pdf" },
    ];
    ...
    const args = input
      .slice(input.indexOf(inputCommand) + inputCommand.length)
      .trim()
      .split(/\s*\|\s*/);
    ...
    if (closestCommand === "view") {
      ...
      if (args[0]) {
        ...
        setPdfs((prevPdfs) =>
          prevPdfs.map((pdf) => ({ ...pdf, selectedPages: new Set() }))
        ); // Deselect all pages
        return;
      }
  ...
  const parseCommand = async (input) => {
    const commandsListString =
      "[" + chatCommands.map((cmd) => cmd.command).join(", ") + "]";
    ...
    const prompt =
      ...
      input;
    const response = await BotFunctions.callApiNoStream(
      prompt,
      settings["Use Api"]
    );
  ...
  function getClosestCommand(inputCommand, commandList) {
    // console.log("inputCommand: ", inputCommand);
    const inputCmdWithoutExclamation = inputCommand.startsWith("!")
    ...
    commandList.forEach((cmdObj) => {
      ...
      if (commandName.startsWith(inputCmdWithoutExclamation)) {
        const lengthDifference =
          commandName.length - inputCmdWithoutExclamation.length;
        if (lengthDifference < closestMatchLengthDifference) {
          closestMatch = cmdObj.command;
          closestMatchLengthDifference = lengthDifference;
        }
      ...
      <div style={predictiveViewStyle}>
        {suggestions.map((suggestion, index) => (
          <div
            ...
            style={
              hoverIndex === index
                ? predictiveViewItemHoverStyle
                : getPredictiveViewItemStyle(index)
            }
            onMouseEnter={() => setHoverIndex(index)}
            onMouseLeave={() => setHoverIndex(null)}
  ...
  const miniViewPdfViewerContainerStyle = {
    flex:
      isMiniViewActive && isPdfViewerActive
        ? 2
        : isMiniViewActive || isPdfViewerActive
        ? 2
        : 0,
          ...
          >
            ...
            >
              <ChatComponent
                ...
                currentBotText={currentBotMessageText}
                wide={settings["Miniview"] || settings["PDF Viewer"]}
              />
            </div>

            <div
              style={{ ...miniViewPdfViewerContainerStyle }}
              className="dark-scrollbar"
            >
            ...
            >
              {suggestions.length > 0 && (
                <PredictiveView suggestions={suggestions} />
              )}
              <TextareaAutosize
                className="dark-scrollbar"
                ref={textareaRef}
                ...
                style={{
                  ...textareaStyles,
                  marginTop: suggestions.length > 0 ? "210px" : "10px",
                  marginLeft: "75px",
                }}
                value={input}
                onChange={handleInputChange}
